...
class PairTradingUpdatePosition(PairTradingIntraDay):
    ...
    def update_future_position(self):
        ...
        """
        if self.signal == 1:
            self.stock1_future_numbers = math.floor((0.5 * self.buying_power) / self.df_signal_summary.iloc[-1]['stock1_final_price'])
            self.stock2_future_numbers = -math.floor((0.5 * self.buying_power) / self.df_signal_summary.iloc[-1]['stock2_final_price'])
        elif self.signal == -1:
            self.stock1_future_numbers = -math.floor((0.5 * self.buying_power) / self.df_signal_summary.iloc[-1]['stock1_final_price'])
            self.stock2_future_numbers = math.floor((0.5 * self.buying_power) / self.df_signal_summary.iloc[-1]['stock2_final_price'])
        else:
